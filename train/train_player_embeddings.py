...
from architectures import EmbeddingModel
import pandas as pd
import numpy as np
...
class PlayerEmbeddings():
    def __init__(self, req_games=5, max_games=100, offset=0, embedding_size=100, lr=0.001, epochs=30, emb_dir='embeddings'):
        """
        ...
        emb_filename = f'emb_size{embedding_size}.json'
        self.emb_file = self.emb_dir / emb_filename
    ...
    def load(self, update=False):
        ...
        """
        if self.emb_file.exists() and not update:
    ...
    def train(self):
        ...
        databuilder = PlayerEmbeddingDatasetBuilder(req_games=self.req_games, max_games=self.max_games, offset=self.offset)
        ...
        model = EmbeddingModel(input_size=input_size, hidden_size=self.embedding_size, output_size=output_size)
